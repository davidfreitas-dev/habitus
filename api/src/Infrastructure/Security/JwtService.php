...
class JwtService
{
    ...
    public function generateAccessToken(int $userId, string $email): string
    {
        ...
        if (!$user instanceof \App\Domain\Entity\User) {
            throw new AuthenticationException('Usuário não encontrado para geração de JWT');
        }
    ...
    public function validateToken(string $token): object
    {
        try {
            ...
            if ($this->isTokenBlocked($decoded->jti)) {
                throw new AuthenticationException('O token foi invalidado');
            }

            return $decoded;
        } catch (ExpiredException $e) {
            throw new AuthenticationException('O token expirou', 401, $e);
        } catch (SignatureInvalidException $e) {
            throw new AuthenticationException('Assinatura de token inválida', 401, $e);
        } catch (Exception $e) {
            throw new AuthenticationException('Token inválido', 401, $e);
        }
    }
