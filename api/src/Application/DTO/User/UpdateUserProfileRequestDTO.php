...
class UpdateUserProfileRequestDTO
{
    ...
    public static function validateCpfCnpj(?string $value, $context): void
    {
        ...
        if (strlen((string) $cpfcnpj) !== 11 && strlen((string) $cpfcnpj) !== 14) {
        ...
        if (preg_match('/^(\d)\1+$/', (string) $cpfcnpj)) {
            $context->buildViolation('CPF/CNPJ inválido.')
                ->addViolation();
        }
    }
    ...
    {
        if (!$this->profileImage instanceof \Psr\Http\Message\UploadedFileInterface) {
            return null;
        }
