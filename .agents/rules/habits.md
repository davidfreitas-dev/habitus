...
- Authentication state (Access Token) is managed by `AuthStore` and injected into requests via the `api` interceptor.
- **Refresh Tokens are strictly handled via `HttpOnly` cookies** generated by the backend, rather than `localStorage`.
- To support `SameSite=Lax` cookies on mobile/web during development, the app communicates with the API using relative paths (e.g., `/api/v1`) routed through a local Nginx proxy, avoiding Cross-Origin (CORS) boundaries.
- The `api` interceptor automatically handles 401 errors by attempting a token refresh (which transparently uses the `HttpOnly` refresh cookie).
- Always use `src/constants/storage.js` for general storage keys (though not for tokens).
