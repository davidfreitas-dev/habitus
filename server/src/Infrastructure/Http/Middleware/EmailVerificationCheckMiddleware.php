    ...
    public function process(Request $request, RequestHandler $handler): Response
    {
        ...
        try {
            if (!isset($jwt->is_verified)) {
                throw new UnexpectedValueException('Payload JWT faltando a reivindicação is_verified');
            }

            if (!$jwt->is_verified) {
                throw new AuthorizationException('E-mail não verificado. O acesso a este recurso é restrito.', 403);
            }
        } catch (UnexpectedValueException $unexpectedValueException) {
            // Log the error for debugging purposes if a claim is missing
            // For now, rethrow as AuthorizationException
            throw new AuthorizationException('Erro de autenticação: ' . $unexpectedValueException->getMessage(), 403);
        }
